...
# hibernatefile - change hibernation image file location. Image may only be located on
# the root volume. Please use caution. (value = path)
# ttyskeepawake - prevent idle system sleep when any tty (e.g. remote login session) is
# 'active'. A tty is 'inactive' only when its idle time exceeds the system sleep timer. (value = 0/1)
# networkoversleep - this setting affects how OS X networking presents shared network
# services during system sleep. This setting is not used by all platforms; changing its value is unsupported.
#
